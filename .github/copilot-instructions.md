...
## Preferred Patterns
- アイコンライブラリは、既存実装と依存関係に合わせて `@mui/icons-material` を優先して提案してください。
- UIコンポーネントは MUI（Material-UI）ベースの構成を優先して提案してください。
- `shadcn/ui` や `Lucide React` など、未導入のライブラリを前提とした提案・実装は避けてください。これらを採用する提案を行う場合は、依存関係の追加が必要であることを明記してください。
